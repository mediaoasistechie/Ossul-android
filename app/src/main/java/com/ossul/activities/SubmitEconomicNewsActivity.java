...
public class SubmitEconomicNewsActivity extends BaseActivity implements INetworkEvent, ImagePickerCallback {
    private static final String TAG = SubmitEconomicNewsActivity.class.getSimpleName();
    ...
    private EconomicNewsList mEconomicNews;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent() != null && getIntent().getBundleExtra("bundle") != null) {
            mEconomicNews = getIntent().getBundleExtra("bundle").getParcelable("data");
        }
        initData();
    }
            ...
            try {
                ...
                if (jsonObject != null && jsonObject.length() != 0) {
                    ...
                    if (!Validator.isEmptyString(AppUtilsMethod.getValueFromKey(jsonObject, "OK")))
                        ok = AppUtilsMethod.getValueFromKey(jsonObject, "OK");
    ...
    protected void initViews() {
        ...
        RelativeLayout cityRL = (RelativeLayout) findViewById(R.id.rl_city);
    ...
    private void setData() {
        if (mEconomicNews == null)
            return;
        mTitleET.setText(mEconomicNews.title);
        mDescriptionET.setText(mEconomicNews.description);
        // TODO: 19-Nov-17
        ArrayList<String> images = new ArrayList<>();
        JsonArray jsonElements = mEconomicNews.images;
        if (jsonElements != null) {
            ...
            if (mEconomicNews.storagePath != null) {
                image1.setVisibility(View.VISIBLE);
                ImageLoader.getInstance().displayImage(mEconomicNews.storagePath, image1);
                ...
                if (getCountryListResponse.data.size() > 0) {
                    ...
                    if (mEconomicNews != null) {
                        for (int i = 0; i < mCountryNameList.size(); i++) {
                            if (mCountryNameList.get(i).equalsIgnoreCase(mEconomicNews.countryName)) {
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.tv_submit:
                ...
                if (mEconomicNews != null) {
                    callEditNewsAPI(mCountryId, mTitleET.getText().toString(), mDescriptionET.getText().toString(), mEconomicNews.economicNewsId);
                } else
                    callAddNewsAPI(mCountryId, mTitleET.getText().toString(), mDescriptionET.getText().toString());
                break;
            case R.id.iv_image1:
    ...
    private void callAddNewsAPI(String countryId, String title, String description) {
        API_ADD_NEWS = AppNetworkConstants.BASE_URL + "user/news.php";
        NetworkService serviceCall = new NetworkService(API_ADD_NEWS, AppConstants.METHOD_POST, SubmitEconomicNewsActivity.this);
        MultipartBuilder multipartBuilder = new MultipartBuilder()
                .type(MultipartBuilder.FORM)
                .addFormDataPart(ParserKeys.action.toString(), AppNetworkConstants.ACTION_SUBMIT_ECONOMIC_NEWS)
    ...
    private void callEditNewsAPI(String countryId, String title, String description, String newsId) {
        ...
        NetworkService serviceCall = new NetworkService(API_EDIT_NEWS, AppConstants.METHOD_POST, SubmitEconomicNewsActivity.this);
        MultipartBuilder multipartBuilder = new MultipartBuilder()
                .type(MultipartBuilder.FORM)
                .addFormDataPart(ParserKeys.action.toString(), AppNetworkConstants.ACTION_EDIT_ECONOMIC_NEWS)
                .addFormDataPart(ParserKeys.title.toString(), title)
                .addFormDataPart(ParserKeys.country_id.toString(), countryId)
                .addFormDataPart(ParserKeys.economic_news_id.toString(), newsId);
        if (!Validator.isEmptyString(description))
            multipartBuilder.addFormDataPart(ParserKeys.description.toString(), description);
        else
            multipartBuilder.addFormDataPart(ParserKeys.description.toString(), "");
